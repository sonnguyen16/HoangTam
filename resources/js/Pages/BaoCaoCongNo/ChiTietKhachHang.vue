...
function closeModal() {
    $('#chitietkhmodal').modal('hide')
}

const phieu_chi = computed(() => {
    return props.khach_hang?.filter(item => item.ma.startsWith('PT'))
})

const hoa_don = computed(() => {
    return props.khach_hang?.filter(item => item.ma.startsWith('XK'))
})

const hdpc = computed(() => {
    return props.khach_hang?.filter(item => item.ngay >= props.ngay_bat_dau && item.ngay <= props.ngay_ket_thuc)
})
...
<template>
    <div id="chitietkhmodal" class="modal fade" tabindex="-1" role="dialog">
        <div class="modal-dialog modal-xl" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body space-y-2 px-4 mt-2">
                    <p v-if="props.khach_hang?.length > 0"><b class="mr-2">Khách hàng:</b>{{ props.khach_hang[0].ten_kh  }}</p>
                    <div class="d-flex space-x-[80px]">
